...
<form action="<?php echo esc_url( get_permalink( $post->ID ) ); ?>" method="post" class="woocommerce-form woocommerce-form-track-order track_order">
	...
	<p class="form-row"><button type="submit" class="<?php echo esc_attr( pediland_form_class( 'button-default', pediland_form_class( 'button-size-sm' ) ) ); ?>" name="track" value="<?php esc_attr_e( 'Track', 'woocommerce' ); ?>"><?php esc_html_e( 'Track', 'woocommerce' ); ?></button></p>
