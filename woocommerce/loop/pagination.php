...
<nav class="woocommerce-pagination mt-8" aria-label="<?php esc_attr_e('Product Pagination', 'woocommerce'); ?>">
    <ul class="inline-flex overflow-hidden rounded-lg border border-slate-200">
        <?php foreach ($links as $link) : ?>
            <?php $is_current = str_contains($link, 'current'); ?>
            <li class="border-r border-slate-200 last:border-r-0">
                <?php
                $classes = $is_current
                    ? 'page-numbers block bg-slate-900 px-4 py-2 text-sm text-white hover:bg-slate-900'
                    : 'page-numbers block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100';
